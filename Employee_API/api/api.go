...
func ReadEmployeesDesignation(c *gin.Context) {
	...
	data := scyllaClient.Query("SELECT designation FROM employee_info").WithContext(c.Request.Context()).Iter()
...
func ReadEmployeesLocation(c *gin.Context) {
	...
	data := scyllaClient.Query("SELECT office_location FROM employee_info").WithContext(c.Request.Context()).Iter()
...
func ReadCompleteEmployeesData(c *gin.Context) {
	...
	var redisData string
	if redisEnabled {
		redisData, redisError = client.CreateRedisClient().HGet(
			requestCtx,
			"employee",
			"all_data",
		).Result()
	...
	data := scyllaClient.Query("SELECT id, name, designation, department, joining_date, address, office_location, status, email, phone_number FROM employee_info").WithContext(c.Request.Context()).Iter()
...
func ReadEmployeeData(c *gin.Context) {
	...
	data := scyllaClient.Query("SELECT * FROM employee_info where id = ?", id).WithContext(c.Request.Context()).Iter()
...
func CreateEmployeeData(c *gin.Context) {
	...
	if err != nil {
		...
		errorResponse(c, "Invalid joining date format")
		return
	}
	...
	// ==========================================
	// IMPORTANT FIX -> CLEAR REDIS CACHE
	// ==========================================

	if redisEnabled {

		err := client.CreateRedisClient().Del(ctx, "employee").Err()

		if err != nil {
			logrus.Warnf("Unable to clear Redis cache: %v", err)
		} else {
			logrus.Infof("Successfully cleared Redis cache")
		}
	}
...
func writeinRedis(cacheKey, cacheValue string) {
	...
	_, err := client.CreateRedisClient().
		HSet(ctx, "employee", cacheKey, cacheValue).
		Result()

	if err != nil {
		logrus.Errorf("Error in writing data to Redis: %v", err)
	}
}
