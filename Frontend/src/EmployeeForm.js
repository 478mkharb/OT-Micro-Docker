...
const FormikApp = withFormik({
  ...
  async handleSubmit(values, { resetForm, setSubmitting }) {
    ...
    try {
      ...
      const employeeResponse = await fetch('/api/v1/employee/create', {
        method: 'POST',
        headers: {
      ...
      // ================================
        ...
        const salaryResponse = await fetch('/api/v1/salary/create/record', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (!salaryResponse.ok) {
        throw new Error('Salary creation failed');
      }

      alert('Employee and Salary created successfully');
